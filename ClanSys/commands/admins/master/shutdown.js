...
module.exports = {
    ...
    async execute(interaction) {
        if (interaction != null || interaction.channel.id != null) {
            ...
            if (dataCommandAdmin.Shutdown === 'false') { dataCommandAdmin = { Shutdown: 'true' }; };
            // Context

            let lang = require(`../../../.${dataLang.Lang}`)
            if (dataCommandAdmin.Shutdown === 'true') {
                ...
                if (permissions.has(PermissionsBitField.Flags.ViewAuditLog) || permissions.has(PermissionsBitField.Flags.ManageChannels)) {
                    if (dataChannelAdmin != null && interaction.channel.id === dataChannelAdmin.ChannelID) {
                        ...
                        await interaction.reply(`${lang.admin.shutdown.shut}`);
                        process.exit(0);
                        // Error Messages
                    } else {
            ...
            console.log(`[${DateTime.utc().toFormat(timeFormat)}][ClanBot] Interaction of Command \'shutdown\' returned \'null / undefined\'.`);
        };
    },
};