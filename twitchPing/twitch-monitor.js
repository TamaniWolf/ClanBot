    ...
    class TwitchMonitor {
        ...
        static start() {
            ...
            if (!this.channelNames.length) {
                console.warn(chalk.cyan('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + '][TwitchMonitor]', chalk.white('No channels configured')));
                return;
            }
            ...
            // Ready!
            console.log(chalk.yellow('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + '][TwitchMonitor]', chalk.white(`Configured stream status polling for channels:`, this.channelNames.join(', '),
            `(${checkIntervalMs}ms interval)`)));
        }

        static refresh(reason) {
            const now = moment();
            console.log(chalk.cyan('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + '][Twitch]', chalk.white(' ▪ ▪ ▪ ▪ ▪ ', `Refreshing now (${reason ? reason : "No reason"})`, ' ▪ ▪ ▪ ▪ ▪ ')));
            ...
            if (this._lastUserRefresh === null || now.diff(moment(this._lastUserRefresh), 'minutes') >= 10) {
                ...
                .catch((err) => { 
                    console.warn(chalk.yellow('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + '][TwitchMonitor]' + chalk.white('Error in users refresh:', err)));
                })
                .then(() => {
            ...
            if (this._pendingGameRefresh) {
                ...
                .catch((err) => { 
                    console.warn(chalk.yellow('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + '][TwitchMonitor]', chalk.white('Error in games refresh:', err)));
                })
                .then(() => {
                    if (this._pendingGameRefresh) {
                        this._pendingGameRefresh = false;
            ...
            if (!this._pendingUserRefresh && !this._pendingGameRefresh) {
                ...
                .catch((err) => {  
                    console.warn(chalk.yellow('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + '][TwitchMonitor]', chalk.white('Error in streams refresh:', err)));
                });
            }
        }
        ...
        static handleUserList(users) {
            ...
            if (namesSeen.length) {
                console.debug(chalk.cyan('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + '][TwitchMonitor]', chalk.white('Updated user info:', namesSeen.join(', '))));
            }
        ...
        static handleGameList(games) {
            ...
            if (gotGameNames.length) {
                console.debug(chalk.cyan('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + '][TwitchMonitor]', chalk.white('Updated game info:', gotGameNames.join(', '))));
            }
        ...
        static handleStreamList(streams) {
            ...
            for (let i = 0; i < nextOnlineList.length; i++) {
                ...
                if (this.activeStreams.indexOf(_chanName) === -1) {
                    ...
                    console.log(chalk.cyan('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + '][TwitchMonitor]', chalk.white('Stream channel has gone online:', _chanName)));
                    anyChanges = true;
                }
            ...
            for (let i = 0; i < this.activeStreams.length; i++) {
                ...
                if (nextOnlineList.indexOf(_chanName) === -1) {
                    ...
                    console.log(chalk.cyan('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + '][TwitchMonitor]', chalk.white('Stream channel has gone offline:', _chanName)));
            ...
            if (!notifyFailed) {
                ...
                console.log(chalk.cyan('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + '][TwitchMonitor]', chalk.white('Could not notify channel, will try again next update.')));
            }
