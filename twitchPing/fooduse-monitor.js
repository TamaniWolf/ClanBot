...
if(configonoff.twitch === true) {
    ...
    class FooduseMonitor {
        ...
        static doCheck() {
            ...
            fpReq.on('error', (err) => {
                console.error(chalk.yellow('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + '][Fooduse]', chalk.white('Could not check Youtube feed (request error).', err)));
            });
            ...
            fp.on('error', (err) => {
                console.error(chalk.yellow('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + '][Fooduse]', chalk.white('Could not check Youtube feed (parser error).', err)));
            });
        ...
        static handleHighestItem(rssItem) {
            ...
            console.debug(chalk.cyan('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + '][Fooduse]', chalk.white(`Found new video to announce: ${rssItem.title} [${rssItem.guid}]`)));
            ...
            for (let i = 0; i < this.targetChannels.length; i++) {
                ...
                if (targetChannel) {
                    try {
                        ...
                        console.error(chalk.yellow('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + '][Fooduse]', chalk.white('Announce error:', err)));
                    }
                }
            }
        }
