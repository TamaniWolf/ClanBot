...
//Start
console.log(chalk.yellow(lang.prefix.royal + chalk.white(' ▪ ▪ ▪ ▪ ▪ ', 'Royal Start', ' ▪ ▪ ▪ ▪ ▪ ')));

//MC Ping Handler
if(configonoff.mcping === true) {
    function getServerStatus() {
        mcping(process.env.IP, 25556, function(err, res) {
...
if (config.set.profilenewjoin === true) {
    client.on("guildMemberAdd", member => {
            // profile to check if exists
            const profile = `./db/economy/profiles/${member.id}.json`;
            // check if profile exists
            if (fs.existsSync(profile)) {
                console.log(chalk.cyan('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + ']' + lang.prefix.royal, `Rejoined '${member.displayName} : ${member.id}'.`));
                return;
            } else {
                // DB Number count
                ...
                fs.appendFile(`./db/economy/profiles/${member.id}.json`, dataprofile, function (err){
                    if (err) throw err;
                console.log(chalk.cyan('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + ']' + lang.prefix.royal, `Profile file '${member.id}.json' is creacted.`));
                });
                ...
                fs.appendFile(`./db/cooldown/${member.id}.json`, datacooldown, function (err){
                    if (err) throw err;
                console.log(chalk.cyan('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + ']' + lang.prefix.royal, `Cooldown file '${member.id}.json' is creacted.`));
                });
            }
    });
} else {
...
class StreamActivity {
    ...
    static updateActivity() {
        ...
        if (streamInfo) {
            this.discordClient.user.setActivity(serverStatus, {
                ...
                try {
                    ...
                    if (existingMsgId) {
                        ...
                        if (mentionMode) {
                            ...
                            if (mentionMode === "Twitch Squad" || mentionMode === "here") {
