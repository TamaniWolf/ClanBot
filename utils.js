...
if(configonoff.mcping === true) {
    module.exports.mcUpdate = function (client, url) { 
        ...
        request(url, function(err, response, body) {
            if(err) {
                console.log(chalk.yellow('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + '][MC] Ping:', chalk.white(err)));
                //return message.reply('Error getting Minecraft server status...');
            }
            body = JSON.parse(body);
            var status = 'Server offline';
            console.log(chalk.yellow('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + '][MC] Ping:', chalk.white(body.motd)));
            if(body.online) {
    ...
    module.exports.log = function (msg) { 
        console.log(chalk.cyan('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + '][MC] Ping:', chalk.white(msg)));
    }

    module.exports.kill = function (code, error) {
        console.log(chalk.yellow('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + '][MC] Ping:', chalk.white(error)));
        process.exit(code);
    }
    ...
    module.exports.ping = async function (host, port, pong) { 
        ...
        if (http.readyState == 4) {
            ...
            console.log(chalk.cyan('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + '][MC] Ping:', chalk.white(`"${host}:${port}" with a delay of ${milliseconds} milliseconds`)));
