    ...
    async execute(client, message, args) {
        if(configonoff.command.user.eco.addprofile === true) {
            ...
            if(message.channel.id === adminchannel1 || message.channel.id === adminchannel2 || message.channel.id === adminchannel3 || message.channel.id === adminchannel4 || message.channel.id === adminchannel5) {
                    ...
                    if (configonoff.command.user.eco.addprofile === true) {
                        // console.log(lang.prefix.royal, `UserID ${member.id}, ${member.displayName}`)
                        ...
                        if (fs.existsSync(profile)) {
                            ...
                            // console.log(chalk.cyan('[' + moment().format('MM/DD/YYYY-h:mm:ss-A') + ']' + lang.prefix.royal, `Profile of '${member.displayName}' not found.`));
                            let profilewrite = {
                            ...
                            fs.appendFile(`./db/cooldown/${member.id}.json`, datacooldown, function (err){
                                if (err) throw err;
                            console.log(chalk.cyan('[' + moment.utc().format('MM/DD/YYYY-h:mm:ss-A') + ']' + lang.prefix.royal, `Cooldown file '${member.is}.json' is creacted.`));
                            });
                        }
                    } else {
                        return;
